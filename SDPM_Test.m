...
J(:,1) = J(:,1) - J(:,end); % Lower TE tangent kutta condition
J(:,numPan) = J(:,numPan) + J(:,end); % Upper TE tangent kutta condition
J_wake = J(:,end); % Wake influence separate column
...
RHS(numPan+1) = 0; % Add numPan + 1 row to RHS column vector to make system of equations solveable
...
Vt_d = J(:, 1:numPan) * mu(1:numPan); % Compute doublet tangent velocity contribution at each panel
